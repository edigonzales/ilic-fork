...
def main() -> int:
    ...
    if "_ilic_editor_snapshot" not in package or "ilic_editor_snapshot" not in capi:
        raise SystemExit("native editor snapshot export is not wired through JS and C ABI")
    if "buildIli2Impl<EditorTokenStream>(source,mode,true)" not in pipeline:
        raise SystemExit("tolerant parser recovery is not isolated in the editor mode")
    if "buildIli2Impl<antlr4::CommonTokenStream>(source,mode,false)" not in pipeline:
