...
self.onmessage = async event => {
  ...
  try {
    ...
    switch (method) {
      ...
      case "editorSnapshot": value = session(args[0]).editorSnapshot(...args.slice(1)); break;
      case "analyze": value = session(args[0]).analyze(...args.slice(1)); break;
      case "compileAndAnalyze": value = session(args[0]).compileAndAnalyze(...args.slice(1)); break;
