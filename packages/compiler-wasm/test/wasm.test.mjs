...
test(
  "compiles and formats through the real WASM ABI",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
  },
  async () => {
...
`,
    ...
    const model = semantic.symbols.find(
      (symbol) => symbol.qualifiedName === "WasmModel",
    );
    assert.ok(model);
    ...
    const topic = semantic.symbols.find(
      (symbol) => symbol.qualifiedName === "WasmModel.Topic",
    );
    assert.ok(topic);
    assert.equal(topic.endRange?.start.line, 7);
    assert.equal(topic.endRange?.start.character, 6);
    const item = semantic.symbols.find(
      (symbol) => symbol.qualifiedName === "WasmModel.Topic.Item",
    );
    assert.ok(item);
    ...
    assert.deepEqual(
      combined.semantic.diagnostics,
      combined.compilation.diagnostics,
    );
    assert.deepEqual(
      combined.syntax.map((snapshot) => snapshot.uri),
      [uri],
    );
    const itemNode = combined.semantic.diagram.nodes.find(
      (node) => node.id === "class:WasmModel.Topic.Item",
    );
    assert.ok(itemNode);
...
`,
    ...
    assert.ok(
      invalid.diagnostics.some(
        (diagnostic) =>
          diagnostic.range?.uri === invalidUri &&
...
test(
  "exposes incremental session statistics through the WASM ABI",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
  ...
  async () => {
    ...
    session.putSource(
      uri,
      `INTERLIS 2.4;
MODEL IncrementalStats AT "https://example.invalid" VERSION "1" =
END IncrementalStats.
`,
      1,
    );
    session.parse(uri);
    const stats = session.incrementalStats();
...
test(
  "validates numeric range bounds through the WASM ABI",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
...
test(
  "keeps the WASM session usable while an editor buffer is incomplete",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
...
test(
  "exports functions and views as stereotyped diagram nodes",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
...
`,
    ...
    const semantic = session.analyze({ roots: [uri] });
    assert.equal(semantic.success, true, JSON.stringify(semantic.diagnostics));
    const modelScope = semantic.diagram.nodes.find(
      (node) => node.kind === "modelScope",
    );
    const topic = semantic.diagram.nodes.find(
      (node) => node.kind === "topic" && node.label === "Data (WasmDiagram)",
    );
    assert.ok(modelScope);
    assert.ok(topic);
    assert.ok(
      semantic.diagram.nodes.some(
        (node) =>
          node.kind === "function" &&
...
test(
  "exports inheritance references for EXTENDED attributes",
  {
    skip: existsSync(modulePath)
      ? false
      : "WASM package artifacts have not been built",
...
`,
    ...
    assert.equal(
      ...
      true,
      JSON.stringify(result.compilation.diagnostics),
    );
    const base = result.semantic.symbols.find(
      (symbol) => symbol.qualifiedName === "Extended.Data.Base.Name",
    );
    const child = result.semantic.symbols.find(
      (symbol) => symbol.qualifiedName === "Extended.Data.Child.Name",
    );
    assert.ok(base);
    assert.ok(child);
    assert.ok(
      result.semantic.references.some(
        (reference) =>
