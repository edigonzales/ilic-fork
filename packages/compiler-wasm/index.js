...
function projectLegacyEditorSnapshot(syntax) {
  ...
  for (const node of syntax.nodes) {
    ...
    const parent = parentNode === null || parentNode === undefined
      ? undefined : declarationByNode.get(parentNode);
  ...
  for (const node of syntax.nodes) {
    ...
    if (syntax.importReferences?.some(reference => legacyContainsRange(reference.range, range))) continue;
    let parentNode = node.parent;
    while (parentNode !== null && parentNode !== undefined
      && !declarationByNode.has(parentNode))
      parentNode = nodesById.get(parentNode)?.parent;
...
export class CompilerSession {
  ...
  editorSnapshot(uri) {
    ...
    return callJson(this.#module, this.#module._ilic_editor_snapshot,
      this.#handle, { schemaVersion: 1, uri });
  }
