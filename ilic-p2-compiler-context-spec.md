...
**Referenzstand der Bestandsaufnahme:** `d57f12942e936c9485bd6494a38b198e255cb144`  
**Zielphase:** P2  
...
**Status dieses Dokuments:** normative Implementierungsspezifikation
...
### 3.2 P1 muss vorhanden sein

Vor Beginn MUSS der Agent verifizieren:

- versioniertes Conformance-Korpus;
- importierte/adaptierte `ili2c`-Fälle;
- Manifestvalidierung;
- Native-Ausführung;
- Native/WASM-Paritätsprüfung;
- Known-Divergence-/XFAIL-/XPASS-Regeln;
- reproduzierbare Coverage- und Importberichte.

Falls P1 im tatsächlichen Branch anders strukturiert ist als in `ilic-p1-conformance-spec.md`, MUSS der Agent die reale Struktur verwenden. Er DARF P1-Tests, Manifeste, Fixtures oder Berichte nicht entfernen, verkleinern oder umgehen.

### 3.3 Ausgangsbaseline
...
Danach MUSS er die vollständige P0-/P1-Suite im unveränderten Ausgangszustand ausführen und die Ergebnisse als Baseline speichern. Ein bereits fehlschlagender Ausgangstest MUSS als solcher dokumentiert werden; er darf nicht still P2 angelastet oder übersprungen werden.
...
PImpl ist verbindlich, sofern P1 nicht bereits eine gleichwertige interne Kapselung eingeführt hat. Eine Abweichung MUSS erklären, wie öffentliche Header vom neuen internen Context/Store entkoppelt bleiben.
...
- Pfade/Zeiten, die bereits vor P2 variabel sind, dürfen nur gemäss bestehender P1-Normalisierung behandelt werden.
...
- Native/WASM-Parität aus P1 muss für alle aktiven Fälle unverändert grün bleiben.
...
P0- und P1-Tests bleiben Bestandteil der normalen Suite.
...
## 44. P1-Regressionsanforderung

Nach jedem wesentlichen Migrationsschritt MUSS die P1-Suite laufen.

Erforderlich:

- alle aktiven unterstützten Conformance-Fälle;
- Diagnosereihenfolge;
- Fehlercodes;
- Ranges;
- Metamodellassertions;
- Native/WASM-Parität;
- Known Defects;
- striktes XPASS.

P2 darf keinen Fall als Known Defect hinzufügen, nur um einen Refactorregressionsfehler zu kaschieren.

Eine echte, bereits vorher verborgene Race-/Lifetime-Korrektur, die beobachtbares Ergebnis ändert, MUSS separat adjudiziert und mit Vorher-/Nachherbeleg dokumentiert werden.
...
Nach jeder Gruppe fokussierte Tests plus P1-Shard.
...
Native Repository darf im TSan-Job nur dann deaktiviert werden, wenn ein belegter Drittanbietergrund besteht und alle Core-/P1-Compilerfälle trotzdem laufen. Dies ist im Workflow zu kommentieren.
...
- TSan-Log;
- fehlgeschlagene Conformance-Diffs;
...
- Alle P0-Tests grün.
- Alle P1-Tests grün.
- Keine neue Known Divergence für Refactorregressionen.
- Native/WASM normalisierte Resultate unverändert.
...
14. Standard-`assert`;
15. Deaktivieren von P0/P1-Fällen;
...
Verwende die realen P1-Befehle. Mindestens:
...
- vollständiger P1-WASM-Conformance-Lauf;
- Native/WASM-Paritätsvergleich;
- vorhandene Snapshot-Consumer-Tests.
...
- [ ] P0 und P1 wurden vor und nach P2 vollständig ausgeführt.
...
18. **TSan-Ergebnisse**
19. **P1-Conformance und Native/WASM-Parität**
20. **Plattformmatrix**
...
Lies zuerst die gesamte Datei `ilic-p2-compiler-context-spec.md`. Sie ist
normativ. Prüfe den Ausgangszustand mit `git status --short`, bewahre fremde
Änderungen und führe die vollständige P0-/P1-Baseline aus.

Beseitige danach den veränderlichen Prozesszustand des Compilercores. Führe
einen pro Lauf erzeugten internen `CompilerContext`, einen eindeutig
besitzenden `MetaModelStore`, einen instanzlokalen `MetaModelBuilder`, einen
besitzenden `IliFileCatalog`, explizite Source-Abhängigkeiten und
instanzlokales Logging ein. Entferne den globalen Compiler-Mutex, globale
Logger-/Metamodell-/IliFile-/Context-/Versionzustände, thread-local
Active-Source-Zugriff, globale Resets und mutable Outputstatics.

Mehrere verschiedene `CompilerSession`s müssen echt parallel und
datenrassenfrei kompilieren; dieselbe Session wird per Sessionmutex
serialisiert. C-ABI und Native/WASM-Verhalten bleiben kompatibel.

Implementiere alle verlangten Architektur-, Ownership-, Lifetime-,
Isolation-, Output- und C-ABI-Concurrency-Tests, den Global-State-Guard und
einen Linux-ThreadSanitizer-Job. Verwende C++17 und das P0-Test-Harness.

Arbeite nicht nur einen Plan aus. Implementiere, baue, teste, korrigiere und
wiederhole, bis Release, Debug, ASan/UBSan/LSan, TSan, P1-Conformance,
Native/WASM-Parität und die Plattformmatrix soweit in der Umgebung ausführbar
erfolgreich sind. Führe keine XFAILs oder lockeren Erwartungen ein, um
Refactorregressionen zu verstecken.

Beende mit dem in der Spezifikation verlangten 25-teiligen Abschlussbericht.
Behaupte keinen Lauf, den du nicht tatsächlich ausgeführt hast.
```
...
9. **P1-Gleichheit vor Refactorästhetik:** keine semantische Drift.
